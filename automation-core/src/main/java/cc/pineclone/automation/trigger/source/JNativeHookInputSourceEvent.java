...
@Builder
@Setter
public class JNativeHookInputSourceEvent {

    private static final Map<KeyOperationPair, JNativeHookInputSourceEvent> CACHE = new ConcurrentHashMap<>();

    // 享元静态工厂方法
    public static JNativeHookInputSourceEvent of(Operation operation, Key key) {
        KeyOperationPair pair = new KeyOperationPair(operation, key);
        return CACHE.computeIfAbsent(pair, k -> new JNativeHookInputSourceEvent(operation, key));
    }

    private Operation operation;  /* 按键为按下状态 */
    private Key key;  /* 描述用户按下的按键 */

    private JNativeHookInputSourceEvent(Operation operation, Key key) {
